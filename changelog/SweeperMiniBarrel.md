...
**Evidence:** static EXT-handler comparison, independently reviewed. No real BLE capture or hardware qualification accompanies this entry. The Prod 1.04 normal GATT path and outer-frame dispatcher now have [additional static evidence](../appendices/k10-barrel-prod104-ingress.md); Test 1.02 registration scope and runtime delivery remain unresolved. This entry still describes the same API-handler comparison, without claiming a working phone-to-barrel repair procedure.
...
`P[2]` is ignored by the Prod selector branch; the data switch is `P[3]`. Its state predicate is evaluated before producing the address payload. The production reference now establishes its complete-frame coordinates separately. That documentation closure is not an additional firmware API change.
...
The Test 1.02 frame relationship is documented in [the exact analyzed baseline](../reference/wocode.md#analyzed-baseline), with [separate production framing evidence](../reference/wocode.md#production-104-normal-gatt). The [reviewed EXT58/EXT59 contract](../appendices/k10-barrel-ext58.md) and [synthetic getter cases](../fixtures/k10-barrel-ext58.json) extend this evidence record. Deferred storage behavior is intentionally outside this API changelog until a corresponding BLE-visible effect is demonstrated.
